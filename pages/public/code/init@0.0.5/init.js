    ...
    for (let cmd of task.run) {
      ...
      if (!DUMMY) {
        ...
        if (pipeinto && pipeinto !== true) {
          ...
          const outlog = await Deno.open(`/var/log/private/${Date.now()}_${task.name}_${cmd[0]}_out.log`, { write: true, create: true });
          const errlog = await Deno.open(`/var/log/private/${Date.now()}_${task.name}_${cmd[0]}_err.log`, { write: true, create: true });
